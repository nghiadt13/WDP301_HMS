...
function generateHtmlContent() {
    const rows = test_cases.map(tc => {
        const [tc_id, module, api, method, desc, pre_cond, inp, expected, status, priority, tester, date, note] = tc;
        const method_class = ['get', 'post', 'put', 'delete'].includes(method.toLowerCase()) ? `method-${method.toLowerCase()}` : 'method-get';
        return `            <tr>
...
function generateMarkdownTracking() {
    const mdLines = [
    ...
    test_cases.forEach(tc => {
        const [tc_id, module, api, method, desc, _, __, ___, status, priority, tester, date] = tc;
        mdLines.push(`- [x] **${tc_id}** [${module}] \`${method} ${api}\` - ${desc} (Tester: \`${tester}\`, Date: \`${date}\`) -> **PASS**`);
    });
...
const markdownContent = generateMarkdownTracking();

fs.writeFileSync(path.join(__dirname, 'TestReport.html'), htmlContent, 'utf-8');
fs.writeFileSync(path.join(__dirname, 'index.html'), htmlContent, 'utf-8');
fs.writeFileSync(path.join(__dirname, 'test-tracking.md'), markdownContent, 'utf-8');
