...
import { useCreateRoom, useRoomTypes } from '../hooks/use-rooms';
import './add-room.css';
...
const AddRoomPage = () => {
  ...
  const [form, setForm] = useState({
    roomName: '',
    room_type_id: preselectedRoomTypeId,
    description: '',
    isActive: true,
  });
  ...
  const handleSubmit = () => {
    ...
    createMutation.mutate(payload, {
      ...
      onError: (err) => {
        const msg = err.response?.data?.message || 'Tạo phòng thất bại. Vui lòng thử lại.';
        alert(msg);
      },
    });
  };
